...
const AgentWorkspace = () => {
  ...
  const handleDictationTranscribed = useCallback((text) => {
    const spokenText = String(text || '').trim();
    if (!spokenText) return;
    setAgentInput((previous) => {
  ...
  const dictationBusy = dictationStatus !== 'idle';
  const composerLocked = selectedChatReplyLocked || dictationBusy;
  const displayedAgentInput = dictationBusy && dictationPartial
    ? `${agentInput}${agentInput && !/\s$/.test(agentInput) ? ' ' : ''}${dictationPartial}`
    : agentInput;
