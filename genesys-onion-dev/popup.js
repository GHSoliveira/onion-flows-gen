...
async function refresh() {
  ...
  $("operator-name").textContent = state.ixcOperator?.techName || "Nao configurado";
...
$("login").addEventListener("click", async () => { const result = await send({ type: "DEV_LOGIN", username: $("username").value, password: $("password").value, baseUrl: baseUrl.value }); if (!result?.ok) alert(result?.error || "Falha no login"); $("password").value = ""; refresh(); });
